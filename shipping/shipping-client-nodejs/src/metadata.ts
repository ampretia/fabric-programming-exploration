...
async function main() {
    ...
    try {
        ...
        const getMetadata = await contract.createTransaction('org.hyperledger.fabric:GetMetadata');

        const jsonStr = (await getMetadata.evaluate()).toString();
        const metadata = JSON.parse(jsonStr);

        jsome(metadata);
        fs.writeFileSync('metadata.json', jsonStr, 'utf8');
    } catch (error) {
